...
export const getSop_men = (count) => (dispatch) => {
  ...
  axios
    .get(`https://wild-blue-swallow-wig.cyclic.app/products?category=mens&_page=1&_limit=${count}`)
...
export const getSop_Women = (count) => (dispatch) => {
    dispatch({ type: FETCH_REQUEST });
    axios
      .get(`https://wild-blue-swallow-wig.cyclic.app/products?category=womens&_page=1&_limit=${count}`)
  ...
  export const getSop_Accesories = (count) => (dispatch) => {
    dispatch({ type: FETCH_REQUEST });
    axios
      .get(`https://wild-blue-swallow-wig.cyclic.app/products?category=mobile_accessories&_page=1&_limit=${count}`)
